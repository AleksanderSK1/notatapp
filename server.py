...
@app.get("/notater")
def hent_notater(x_api_key: str = Header()):
    sjekk_api_key(x_api_key)
    return les_data()["notater"]

@app.post("/notater")
def nytt_notat(data: Notat, x_api_key: str = Header()):
    sjekk_api_key(x_api_key)
    d = les_data()
...
@app.patch("/notater/{i}")
def endre_notat(i: int, data: Notat, x_api_key: str = Header()):
    sjekk_api_key(x_api_key)
    d = les_data()
    if i >= len(d["notater"]):
...
def slett_notat(i: int, x_api_key: str = Header()):
    sjekk_api_key(x_api_key)
    d = les_data()
    if i >= len(d["notater"]):
        raise HTTPException(status_code=404, detail="Notat ikke funnet")
...
@app.get("/todolister")
def hent_todolister(x_api_key: str = Header()):
    sjekk_api_key(x_api_key)
    return les_data()["todolister"]

@app.post("/todolister")
def ny_todoliste(data: TodoListe, x_api_key: str = Header()):
    sjekk_api_key(x_api_key)
    d = les_data()
...
def endre_todoliste(i: int, data: dict, x_api_key: str = Header()):
    sjekk_api_key(x_api_key)
    d = les_data()
...
def slett_todoliste(i: int, x_api_key: str = Header()):
    sjekk_api_key(x_api_key)
    d = les_data()
    if i >= len(d["todolister"]):
        raise HTTPException(status_code=404, detail="Todoliste ikke funnet")
